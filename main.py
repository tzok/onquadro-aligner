...
def align_sequences(seq1, seq2):
    ...
    for i in range(1, m+1):
        for j in range(1, n+1):
            ...
            if i > 1 and j > 1 and seq1[i-2] == 'G' and seq2[j-2] == 'G' and seq1[i-1] == 'G' and seq2[j-1] == 'G':
                consecutive_g_count = g_count_matrix[i-1][j-1]
            
            # Calculate scores for each possible move
            if seq1[i-1] == 'G' and seq2[j-1] == 'G':
                diagonal_score = score_matrix[i-1][j-1] + calculate_score(seq1, seq2, i-1, j-1, consecutive_g_count)
                # Update G count for this position
                g_count_matrix[i][j] = g_count_matrix[i-1][j-1] + 1
            else:
                diagonal_score = score_matrix[i-1][j-1] + calculate_score(seq1, seq2, i-1, j-1, 0)
...
def display_alignment(aligned_seq1, aligned_seq2):
    ...
    for i in range(len(aligned_seq1)):
        if i < len(aligned_seq2) and aligned_seq1[i] == aligned_seq2[i] and aligned_seq1[i] != '-':
            if aligned_seq1[i] == 'G':
