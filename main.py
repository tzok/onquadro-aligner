...
def calculate_score(seq1, seq2, i, j, consecutive_g_count, quad_structure=None):
    ...
    if seq1[i].upper() == seq2[j].upper():
        ...
        if seq1[i].upper() == "G":
            ...
            if (
                quad_structure
                and j < len(quad_structure)
                and quad_structure[j].isalpha()
            ):
...
def align_sequences(
                ...
                if is_match:  # Match
                    ...
                    if is_g_match:
                        ...
                        if (
                            ...
                            move_score += (
                                3  # Higher bonus for G with letter in structure
                            )
                            ...
                            if (
                                ...
                                if (
                                    j - 2 < len(quad_structure)
                                    and quad_structure[j - 2].isalpha()
                                    and quad_structure[j - 2].lower()
                                    == quad_structure[j - 1].lower()
                                ):
                                    move_score += (
                                        5  # Bonus for matching the same letter
                                    )
                else:  # Mismatch
                    move_score = -1
    ...
    for aligned_seq1, aligned_seq2, _ in complete_alignments:
        # Create a mapping of positions in aligned_seq2 to original positions in seq2
        orig_positions = {}
        orig_pos = 0
...
def calculate_alignment_score(
    ...
    Args:
        ...
        quad_structure: Optional structure string from quadruplex
        orig_positions: Optional mapping of positions in aligned_seq2 to original quadruplex positions
        ...
        if is_match:
            ...
            if aligned_seq1[i] == "G" and aligned_seq2[i] == "G":
                ...
                if quad_structure and orig_positions and aligned_seq2[i] != "-":
                    ...
                    if (
                        orig_pos >= 0
                        and orig_pos < len(quad_structure)
                        ...
                        if (
                            prev_structure_letter
                            and prev_structure_letter == current_letter
                        ):
                            # Extra bonus for matching the same letter in structure
                            base_score += 5
...
def compute_alignment_score_matrix(seq1, seq2, quad_structure=None):
    """
...
def display_ranked_alignments(ranked_alignments, top_n=10):
    ...
    for i, (
        ...
        # Create a match line with special indicators for G matches with non-dot structure
        match_line = ""
